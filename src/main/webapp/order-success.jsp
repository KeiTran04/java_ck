...
<head>
    ...
    <title>Đặt hàng thành công - Shop</title>
...
<body class="d-flex flex-column min-vh-100">
    ...
    <div class="container my-4 flex-grow-1">
        ...
        <div class="success-message card shadow-sm p-5 mx-auto" style="max-width:500px;">
            <i class="bi bi-check-circle-fill text-success" style="font-size:4rem;"></i>
            <h2 class="text-success mt-3">Đặt hàng thành công!</h2>
            <p class="text-muted">Cảm ơn bạn đã mua hàng.</p>
            <c:if test="${not empty orderId}">
                <p>Mã đơn hàng: <strong>#${orderId}</strong></p>
                <p>Tổng tiền: <strong class="text-danger"><fmt:formatNumber value="${total}" pattern="#,###" /> VND</strong></p>
                <p class="text-muted"><i class="bi bi-envelope"></i> Hóa đơn sẽ được gửi đến email.</p>
                <c:if test="${not empty paymentMethod}">
                    <p class="badge bg-info fs-6">Phương thức: ${paymentMethod == 'COD' ? 'Thanh toán khi nhận hàng' : paymentMethod == 'BANK' ? 'Chuyển khoản' : 'Ví MoMo'}</p>
                </c:if>
            </c:if>
            <a href="${pageContext.request.contextPath}/home" class="btn btn-primary mt-2"><i class="bi bi-shop"></i> Tiếp tục mua sắm</a>
        </div>
    </div>
